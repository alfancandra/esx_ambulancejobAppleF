...
function OpenMobileAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = ('Keluarkan Dari Kendaraan'), value ='out_vehicle'}
			}}, function(data, menu)
				...
				if closestPlayer == -1 or closestDistance > 1.0 then
					...
					if data.current.value == 'revive' then
						IsBusy = true
								...
								if IsPedDeadOrDying(closestPlayerPed, 1) then
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', _U('ambulance'), 100)

									-- Show revive award?
									if Config.ReviveReward > 0 then
										ESX.ShowNotification(_U('revive_complete_award', GetPlayerName(closestPlayer), Config.ReviveReward))
										--exports['mythic_notify']:DoHudText('inform', _U('revive_complete_award', GetPlayerName(closestPlayer), Config.ReviveReward), 5000)
									else
										ESX.ShowNotification(_U('revive_complete', GetPlayerName(closestPlayer)))
										--exports['mythic_notify']:DoHudText('inform', _U('revive_complete', GetPlayerName(closestPlayer)), 2500, { ['background-color'] = '#008000', ['color'] = '#ffffff' })
									end
								else
									ESX.ShowNotification(_U('player_not_unconscious'))
									--exports['mythic_notify']:DoHudText('inform', _U('player_not_unconscious'), 2500, { ['background-color'] = '#FF0000', ['color'] = '#ffffff' })
								end
					...
					elseif data.current.value == 'big' then
						...
						if health > 0 then
							...
							TriggerServerEvent('esx_ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
							ESX.ShowNotification(_U('heal_complete', GetPlayerName(closestPlayer)))
							TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ambulance', _U('ambulance'), 50)
							isBusy = false
						else
							ESX.ShowNotification(_U('player_not_conscious'))
						end
		...
		elseif data.current.value == 'faktur' then

						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'faktur', {
							...
							if amount == nil or amount < 0 then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									exports['mythic_notify']:DoHudText('inform', _U('no_players_nearby'), 2500, { ['background-color'] = '#FF0000', ['color'] = '#ffffff' })
								else
									menu.close()
		...
		elseif data.current.value == 'on_duty' then
			TriggerServerEvent('AnnounceEMSOn')
		elseif data.current.value == 'off_duty' then
			TriggerServerEvent('AnnounceEMSOff')
		end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'Pharmacy' then
					if ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'chief_doctor' then
						OpenPharmacyMenu()
					else
						ESX.ShowNotification('Hanya Atasan yang Bisa Mengambil')
					end
				...
				elseif CurrentAction == 'Boats' then
					OpenVehicleSpawnerMenu('boat', CurrentActionData.hospital, CurrentAction, CurrentActionData.partNum)
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		elements = {
			...
			{label = ('Tiket Operasi Plastik'), item = 'tiket_oplas', type = 'slider', value = 1, min = 1, max = 100},
	}}, function(data, menu)
