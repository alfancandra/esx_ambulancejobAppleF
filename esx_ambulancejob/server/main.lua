...
ESX.RegisterServerCallback('esx_ambulancejob:cekEms', function(source, cb)
	CountEMS()
	if EmsConnected < Config.ButuhPolisi then
		cb(false)
	else
		cb(true)
	end
...
RegisterServerEvent('AnnounceEMSOn')
AddEventHandler('AnnounceEMSOn', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~EMS announcement', 'EMS Sedang ON DUTY! Kesehatanmu Yang Utama!', 'CHAR_CALL911', 8)
	end
end)

RegisterServerEvent('AnnounceEMSOff')
AddEventHandler('AnnounceEMSOff', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'EMS', '~b~EMS announcement', 'EMS SEDANG OFF DUTY!', 'CHAR_CALL911', 8)
	end
end)
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName, amount)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage' and itemName ~= 'tiket_oplas') then
		print(('[esx_ambulancejob] [^2INFO^7] "%s" attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
